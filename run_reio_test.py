...
def create_test_yamls():
    if not os.path.exists(REIO_MODEL_YAML):
        ...
        with open(REIO_MODEL_YAML, 'w') as f:
            yaml.dump({
                'reio_model': {
                    'theory_model': 'tanh_model',  # Specifica quale modello usare da reio_models
                    'z_min': 0.0,
                    'z_max': 30.0,
                    'z_points': 200,
                    'params': {'z_re': 7.5, 'delta_z': 0.5}
                }
            }, f)
...
def load_xe_model_from_reio_models(model_name):
    ...
    if reio_models is None:
        raise RuntimeError("reio_models module not available. Cannot load theory models.")
...
def run_test():
    ...
    params = reio_conf.get('params', {'z_re': 8.0, 'delta_z': 0.5})
    ...
    try:
        ...
        print(f"    Make sure {model_type}(z, z_re, delta_z) has the correct signature in reio_models.py")
        return
    ...
    try:
        ...
        if log_prob == 0.0:
            print("⚠️  WARNING: Log-Likelihood is exactly 0.0. Did it actually compare data?")
        elif log_prob == -np.inf:
            print("⚠️  WARNING: Log-Likelihood is -inf.")
